...
async function getBike(id: string): Promise<Bike | null> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/bikes/${id}`, {
    cache: "no-store",
  });
...
export default async function RentalBikeDetailPage({
  params,
}: {
      ...
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2">
        ...
        <div>
          ...
          <Link
            ...
            className="mt-8 inline-flex rounded-full bg-white px-8 py-4 text-sm font-bold uppercase tracking-[0.15em] text-black transition hover:bg-neutral-200"
          >
            Book Now
          </Link>
